...
/// </summary>
public class TrackMap : IMapBuffer
{
    ...
    public void SetBytes(byte[] data)
    {
        if (data.Length != SquareSize)
            throw new ArgumentException($"The map array must have a length of {SquareSize} ({Size} * {Size}).",
                nameof(data));

        for (var y = 0; y < _map.Length; y++) _map[y] = Utilities.ReadBlock(data, y * _map.Length, _map.Length);
    }
    ...
    {
        if (SetTileInternal(x, y, tile)) OnDataChanged();
    }
    ...
    public void SetTiles(int startX, int startY, IMapBuffer tileBuffer)
    {
        ...
        for (var y = 0; y < yLimit; y++)
        {
            ...
            for (var x = 0; x < xLimit; x++)
            {
                ...
                if (SetTileInternal(positionX, positionY, tileBuffer[x, y])) dataChanged = true;
            }
        }
    ...
    {
        for (var x = 0; x < Width; x++) _map[0][x] = tile;

        for (var y = 1; y < Height; y++) Buffer.BlockCopy(_map[0], 0, _map[y], 0, Width);
    ...
    public byte[] GetBytes()
    {
        ...
        for (var y = 0; y < Height; y++) Buffer.BlockCopy(_map[y], 0, data, y * Width, Width);
