...
/// </summary>
public class GPStartPosition : INotifyPropertyChanged
{
        ...
        set
        {
            ...
            if (_enableBoundsChecks)
            {
                if (SecondRowOffset > 0)
                {
                    if (x < Tile.Size)
                        x = Tile.Size;
                    else if (x + SecondRowOffset > PixelLimit) x = PixelLimit - SecondRowOffset;
                }
                else
                {
                    if (x + SecondRowOffset < Tile.Size)
                        x = Tile.Size - SecondRowOffset;
                    else if (x > PixelLimit) x = PixelLimit;
                }

                if (y < Tile.Size)
                    y = Tile.Size;
                else if (y > PixelLimit - Height) y = PixelLimit - Height;
            }
        ...
        set
        {
            if (_enableBoundsChecks)
            {
                if (X + value < Tile.Size)
                    ...
                    value = SecondRowMin;
                else if (value > SecondRowMax) value = SecondRowMax;
            }
