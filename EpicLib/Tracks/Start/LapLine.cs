...
namespace EpicLib.Tracks.Start;
...
public class LapLine
{
    ...
    public Point Location
    {
        ...
        set
        {
            ...
            if (x < 0)
                x = 0;
            else if (x + Length > TrackMap.Size * Tile.Size) x = TrackMap.Size * Tile.Size - Length;

            if (y < 0)
                y = 0;
            else if (y >= TrackMap.Size * Tile.Size) y = TrackMap.Size * Tile.Size - 1;
    ...
    {
        if (point.X <= X + (Tile.Size - 1)) return ResizeHandle.Left;

        if (point.X >= Right - Tile.Size) return ResizeHandle.Right;
    ...
    public void Resize(ResizeHandle resizeHandle, int x)
    {
        ...
        {
            if (Right - x <= 0) x = Right - PrecisionX;
    ...
    public byte[] GetBytes()
    {
        ...
        if (areaY + areaHeight > trackHeight) areaHeight = trackHeight - areaY;
