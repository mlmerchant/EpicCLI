...
/// </summary>
public class TrackAIElement : INotifyPropertyChanged
{
    /// <summary>
    ...
    public TrackAIElementShape AreaShape
    {
        ...
        set
        {
                ...
                value != TrackAIElementShape.Rectangle)
            {
                ...
                else if (_area.Height > _area.Width) _area.Height = _area.Width;
            }
    ...
    public bool IsIntersection
    {
        ...
        set
        {
            if (_isIntersection == value) return;
    ...
    {
        if (AreaShape == TrackAIElementShape.Rectangle) return IntersectsWithRectangle(point);
    ...
    {
        if (!IntersectsWithRectangle(point)) return false;
    ...
    {
        if (AreaShape == TrackAIElementShape.Rectangle) return GetResizeHandleRectangle(point);
    ...
    private ResizeHandle GetResizeHandleTriangle(Point point)
    {
        ...
        switch (AreaShape)
        {
            case TrackAIElementShape.TriangleTopLeft:
                ...
                if (diagonal >= _area.Width - Precision && diagonal <= _area.Width) return ResizeHandle.BottomRight;

                if (point.X == _area.Left) return ResizeHandle.Left;

                if (point.Y == _area.Top) return ResizeHandle.Top;
            ...
            case TrackAIElementShape.TriangleTopRight:
                ...
                diagonal = point.X - _area.X - (point.Y - _area.Y);
                if (diagonal >= -Precision && diagonal <= 0) return ResizeHandle.BottomLeft;

                if (point.X == _area.Right - 1) return ResizeHandle.Right;
            ...
            case TrackAIElementShape.TriangleBottomRight:
                ...
                diagonal = point.X - _area.X + (point.Y - _area.Y);
                if (diagonal >= _area.Width - Precision && diagonal <= _area.Width) return ResizeHandle.TopLeft;

                if (point.X == _area.Right - 1) return ResizeHandle.Right;

                if (point.Y == _area.Bottom - 1) return ResizeHandle.Bottom;
            ...
            case TrackAIElementShape.TriangleBottomLeft:
                ...
                diagonal = point.X - _area.X - (point.Y - _area.Y);
                if (diagonal >= 0 && diagonal <= Precision) return ResizeHandle.TopRight;
    ...
    private void MoveTo(int x, int y)
    {
        ...
        if (x < 0)
            x = 0;
        else if (x + _area.Width > TrackMap.Size) x = TrackMap.Size - _area.Width;

        if (y < 0)
            y = 0;
        else if (y + _area.Height > TrackMap.Size) y = TrackMap.Size - _area.Height;
    ...
    private void MoveTargetTo(int x, int y)
    {
        if (x < 0)
            x = 0;
        else if (x > TrackMap.Limit) x = TrackMap.Limit;

        if (y < 0)
            y = 0;
        else if (y > TrackMap.Limit) y = TrackMap.Limit;
    ...
    private void ResizeRectangle(ResizeHandle resizeHandle, int x, int y)
    {
        ...
        switch (resizeHandle)
        {
            case ResizeHandle.TopLeft:
                ...
                if (x >= _area.Right) x = _area.Right - Precision;

                if (y >= _area.Bottom) y = _area.Bottom - Precision;
        ...
        switch (resizeHandle)
        {
            case ResizeHandle.TopLeft:
                ...
                else
                {
                    ...
                    if (offBounds > 0) length -= offBounds;
                }
