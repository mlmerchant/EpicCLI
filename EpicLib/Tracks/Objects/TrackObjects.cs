...
/// </summary>
public class TrackObjects : IEnumerable<TrackObject>, INotifyPropertyChanged
{
    ...
    {
        foreach (var tObject in _objects) yield return tObject;
    }
    ...
    {
        if (data.Length != Size) throw new ArgumentException("Incorrect track object data size", nameof(data));

        for (var i = 0; i < RegularObjectCount; i++)
        {
            if (_objects[i] != null) _objects[i].PropertyChanged -= SubPropertyChanged;
    ...
    public byte[] GetBytes()
    {
        ...
        for (var i = 0; i < _objects.Length; i++) _objects[i].GetBytes(data, i * BytesPerObject);
