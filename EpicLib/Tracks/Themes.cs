...
/// </summary>
public sealed class Themes : IEnumerable<Theme>, INotifyPropertyChanged, IDisposable
{
    ...
    public void Dispose()
    {
        foreach (var theme in _themes) theme.Dispose();
    ...
    {
        foreach (var theme in _themes) yield return theme;
    }
    ...
    private void Init(byte[] romBuffer, Offsets offsets, TextCollection names)
    {
        ...
        int[] reorder =
            { 5, 4, 6, 9, 8, 10, 7, 12 }; // To reorder the themes, as they're not in the same order as the names

        var paletteOffsets = Utilities.ReadBlockOffset(romBuffer, offsets[Offset.ThemePalettes], _themes.Length);
        var roadTileGfxOffsets =
            Utilities.ReadBlockOffset(romBuffer, offsets[Offset.ThemeRoadGraphics], _themes.Length);
        var bgTileGfxOffsets =
            Utilities.ReadBlockOffset(romBuffer, offsets[Offset.ThemeBackgroundGraphics], _themes.Length);
        var bgLayoutOffsets =
            Utilities.ReadBlockOffset(romBuffer, offsets[Offset.ThemeBackgroundLayouts], _themes.Length);
        ...
        else
        {
            ...
            roadTileGenreIndexes =
                Utilities.ReadBlockGroup(romBuffer, offsets[Offset.TileGenreIndexes], 2, Theme.Count * 2);
        ...
        for (var i = 0; i < _themes.Length; i++)
        {
            ...
            else
            {
                ...
                Buffer.BlockCopy(commonRoadTilePaletteIndexes, 0, allRoadTilePaletteIndexes, RoadTileset.ThemeTileCount,
                    RoadTileset.CommonTileCount);
                ...
                for (var j = 0; j < commonRoadTileGfxClone.Length; j++)
                    commonRoadTileGfxClone[j] =
                        Utilities.ReadBlock(commonRoadTileGfx[j], 0, commonRoadTileGfx[j].Length);

                Array.Copy(commonRoadTileGfxClone, 0, allRoadTileGfx, RoadTileset.ThemeTileCount,
                    commonRoadTileGfxClone.Length);

                // Set empty tile default graphics value
                for (var j = roadTileGfx.Length; j < RoadTileset.ThemeTileCount; j++) allRoadTileGfx[j] = new byte[32];
            }
            ...
            else
            {
                ...
                Array.Copy(commonRoadTileGenres, 0, allRoadTileGenres, RoadTileset.ThemeTileCount,
                    commonRoadTileGenres.Length);
    ...
    {
        foreach (var theme in _themes) theme.PropertyChanged += OnPropertyChanged;
    }
    ...
    private static RoadTileGenre[] GetTileGenres(byte[] data, int start, int size)
    {
        ...
        for (var i = 0; i < size; i++) tileGenres[i] = (RoadTileGenre)data[start + i];
    ...
    private static byte[] GetPaletteIndexes(byte[] tileData, int count)
    {
        ...
        for (var i = 0; i < count; i++) paletteIndexes[i] = (byte)(paletteIndexes[i] >> 4);

        return paletteIndexes;
    }

    private static RoadTileset GetRoadTileset(Palettes palettes, byte[] tilePaletteIndexes, byte[][] tileGfx,
        RoadTileGenre[] tileGenres)
    {
    ...
    {
        foreach (var theme in _themes) theme.ResetModifiedState();
    }
}