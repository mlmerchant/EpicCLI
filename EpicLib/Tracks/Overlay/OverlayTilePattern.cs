...
public class OverlayTilePattern : IEquatable<OverlayTilePattern>
{
    ...
    {
        if (data.Length != size.Width * size.Height) throw new ArgumentException("Data does not match size.");
    ...
    private void SetBytes(byte[] data)
    {
        ...
        for (var y = 0; y < Height; y++) _tiles[y] = Utilities.ReadBlock(data, y * Width, Width);
    }
    ...
    public byte[] GetBytes()
    {
        ...
        for (var y = 0; y < Height; y++) Buffer.BlockCopy(_tiles[y], 0, buffer, y * Width, Width);
