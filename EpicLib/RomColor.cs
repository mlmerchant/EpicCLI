...
public struct RomColor
{
    ...
    {
        if (data == null) throw new ArgumentNullException(nameof(data), "Color byte data cannot be null.");
    ...
    /// </summary>
    public static bool operator ==(RomColor color1, RomColor color2)
    {
    ...
    {
        var stepR = (target._red - (double)_red) / (step - 1);
        var stepG = (target._green - (double)_green) / (step - 1);
        var stepB = (target._blue - (double)_blue) / (step - 1);
    ...
    {
        if (hex.Length != 6) hex = Convert.ToInt32(hex, 16).ToString("X6", CultureInfo.InvariantCulture);
