...
/// </summary>
public class TextCollection : IEnumerable<TextItem>, ITextCollection, INotifyPropertyChanged
{
    /// <summary>
    ...
    /// <param name="hasPaletteData">
    ///     True if the text data is mixed with color palette indexes (ie: each letter followed by the
    ///     index of its associated palette).
    /// </param>
    /// <param name="fixedLength">
    ///     True if the length of each text is specified separately in a length table.
    ///     Otherwise each text item ends with 0xFF.
    /// </param>
    /// <param name="japAltMode">True for Japanese texts with ten-ten and maru characters encoded separately.</param>
    /// <param name="tallCharacters">
    ///     True if characters are tall (two 8x8 vertically-laid tiles), false otherwise (one 8x8 tile
    ///     per character).
    /// </param>
        ...
        bool fixedLength, bool japAltMode, bool tallCharacters, byte shiftValue, byte[] keys, char[] values)
    {
        ...
        if (hasPaletteData) _colorIndexes = new byte[count];

        MaxCharacterCount = GetCharacterCount(_totalSize);

        if (keys != null) Converter.ReplaceKeyValues(keys, values);
        ...
        for (var i = 0; i < offsets.Length; i++)
        {
            ...
            if (offsets[i] < _textOffset) _textOffset = offsets[i];
        }
        ...
        for (var i = 0; i < _texts.Length; i++)
        {
            ...
            if (japAltMode)
            {
                ...
                var tenMaruOffset = Utilities.BytesToOffset(romBuffer[tenMaruIndexOffset],
                    romBuffer[tenMaruIndexOffset + 1], leadingOffsetByte);
    ...
    {
        foreach (var text in _texts) yield return text;
    }
    ...
    private static void SetBytes(byte[] data, byte[] textBytes, ref int index, bool hasPaletteData)
    {
        ...
        if (hasPaletteData) data[index++] = 0xFF;
    }
