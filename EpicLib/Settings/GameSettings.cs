...
public class GameSettings : INotifyPropertyChanged
{
    ...
    private void Init(byte[] romBuffer, Region region)
    {
        ...
        var textDataSizes =
            isJap ? new[] { 48, 94, 70, 68, 144, 136, 96, 42 } : new[] { 66, 130, 90, 80, 173, 134, 112, 52 };
    ...
    {
        if (GPCupSelectTexts != null) GPCupSelectTexts.PropertyChanged += OnPropertyChanged;
    ...
    {
        if (GPCupSelectTexts != null) GPCupSelectTexts.Save(romBuffer);
    ...
    {
        if (GPCupSelectTexts != null) GPCupSelectTexts.ResetModifiedState();
