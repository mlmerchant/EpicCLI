...
internal class Decompressor : IDecompressor
{
    ...
    private void Decompress(byte[] srcBuffer, byte[] destBuffer, byte value, bool throwOnInvalidData, ref int srcOffset,
        ref int destOffset)
    {
        ...
        try
        {
            switch (command)
            {
                case 0: // Reads an amount of bytes from ROM in sequence stores them in the same sequence.
                    for (i = 0; i < count; i++) destBuffer[destOffset++] = srcBuffer[srcOffset++];
                    break;
                ...
                case 3
                    ...
                    for (i = 0; i < count; i++) destBuffer[destOffset++] = (byte)((srcBuffer[srcOffset] + i) & 0xFF);
                    srcOffset++;
                    break;
                ...
                case 4
                    ...
                    for (i = 0; i < count; i++) destBuffer[destOffset++] = destBuffer[prevOffset + i];
                    break;
                ...
                case 5
                    ...
                    for (i = 0; i < count; i++) destBuffer[destOffset++] = (byte)(destBuffer[prevOffset + i] ^ xor);
                    xor = 0x00;
                    break;
                ...
                case 6
                    ...
                    for (i = 0; i < count; i++) destBuffer[destOffset++] = destBuffer[prevOffset++];
                    break;

                default:
                    if (throwOnInvalidData) throw new InvalidDataException("The data can't be decompressed.");
                    break;
            }
        }
        catch (IndexOutOfRangeException)
        {
            destOffset--;
            for (; i < count; i++) destBuffer[destOffset++] = (byte)(0 ^ xor);
        }
    }
}