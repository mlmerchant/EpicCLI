...
internal static class Codec
{
    ...
    private static ICompressor OptimalCompressor =>
        _optimalCompressor ?? (_optimalCompressor = new OptimalCompressor());
    ...
    {
        if (byteCount > SuperCommandMax) byteCount = SuperCommandMax;
    ...
    public static byte[] Decompress(byte[] buffer, int offset, bool twice)
    {
        ...
        if (twice) data = Decompressor.Decompress(data);
