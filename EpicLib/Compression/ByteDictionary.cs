...
internal class ByteDictionary
{
    ...
    public ByteDictionary(byte[] buffer)
    {
        ...
        for (var i = 0; i < buffer.Length; i++) Add(buffer[i], i);
    }
    ...
    public Range GetMaxBackRange(int offset)
    {
        ...
        foreach (var otherOffset in _byteDictionary[value])
        {
            ...
            if (backRange.Length >= maxBackRange.Length) maxBackRange = backRange;
        }
    ...
    public Range[] GetMaxBackRanges(int offset)
    {
        ...
        foreach (var otherOffset in _byteDictionary[value])
        {
            ...
            {
                if (backRange.Length >= maxRange4s.Length) maxRange4s = backRange;
            }
            else
            {
                if (backRange.Length >= maxRange6s.Length) maxRange6s = backRange;
            }
        }
        ...
        if (maxRange4n.Length == maxRange6n.Length) maxRange4n = Range.Empty;

        if (maxRange4s.Length == maxRange6s.Length) maxRange4s = Range.Empty;
