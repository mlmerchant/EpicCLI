...
internal static class Utilities
{
    ...
    /// <param name="buffer">The buffer to retrieve data from.</param>
    /// <param name="offset">The buffer position the function starts reading from.</param>
    /// <param name="stopValues">
    ///     The function keeps reading the buffer until it reaches these values. Null if there is no stop
    ///     value.
    /// </param>
    /// <returns>An array of bytes.</returns>
    public static byte[] ReadBlockUntil(byte[] buffer, int offset, params byte[] stopValues)
    {
        ...
        {
            length = 0;
            while (!IsBlockLimitReached(buffer, offset + length, stopValues)) length++;
        }
    ...
    /// <summary>
    ///     Reads a part of the ROM, dividing the result by groups of a certain size, until we reach a certain amount of
    ///     groups.
    /// </summary>
    /// <param name="buffer">The buffer to retrieve data from.</param>
    /// <param name="offset">The buffer position the function starts reading from.</param>
    ...
    public static byte[][] ReadBlockGroup(byte[] buffer, int offset, int groupSize, int groupCount)
    {
        ...
        for (var i = 0; i < groupCount; i++) group[i] = ReadBlock(buffer, offset + i * groupSize, groupSize);
    ...
    ///     The function keeps reading the buffer until it reaches this value. -1 means there is no stop
    ///     value.
    /// </param>
    /// <param name="groupSize">Defines the number of bytes per group.</param>
    /// <returns>A jagged array that contains byte groups.</returns>
    public static byte[][] ReadBlockGroupUntil(byte[] buffer, int offset, int stopValue, int groupSize)
    {
        ...
        {
            length = 0;
            while (buffer[offset + length] != stopValue) length++;
        }
        ...
        for (var i = 0; i < length; i++) group[i] = ReadBlock(buffer, offset + i * groupSize, groupSize);
    ...
    public static int[] ReadBlockOffset(byte[] buffer, int offset, int offsetCount)
    {
        ...
        for (var i = 0; i < offsetCount; i++) offsetGroup[i] = BytesToOffset(buffer, offset + i * offsetSize);
    ...
    {
        if (data > 0xFFFFF) throw new ArgumentOutOfRangeException(nameof(data), "The offset value is too high.");
    ...
    public static void LoadBytesFromHexString(byte[] bytes, string hex)
    {
        ...
        for (var i = 0; i < bl; ++i)
        {
            bytes[i] = (byte)((hex[2 * i] > 'F' ? hex[2 * i] - 0x57 :
                hex[2 * i] > '9' ? hex[2 * i] - 0x37 : hex[2 * i] - 0x30) << 4);
            bytes[i] |= (byte)(hex[2 * i + 1] > 'F' ? hex[2 * i + 1] - 0x57 :
                hex[2 * i + 1] > '9' ? hex[2 * i + 1] - 0x37 : hex[2 * i + 1] - 0x30);
        }
    }
    ...
    public static int GetColorIndex(Color color, Palette palette)
    {
        ...
        for (var i = 0; i < colors.Length; i++) colors[i] = palette[i];
    ...
    public static int GetColorIndex(RomColor color, RomColor[] colors)
    {
        ...
        if (colorIndex == -1) colorIndex = GetClosestColorIndex(color, colors);
