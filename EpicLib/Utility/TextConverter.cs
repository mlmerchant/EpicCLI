...
public class TextConverter
{
    ...
    public void ReplaceKeyValues(byte[] keys, char[] values)
    {
        ...
        {
            if (_dictionary.Forward.ContainsKey(keys[i])) _dictionary.Remove(keys[i]);

            if (_dictionary.Reverse.TryGetValue(values[i], out var key)) _dictionary.Remove(key);
    ...
    {
        return !_dictionary.Forward.TryGetValue(charByte, out var value) ? '?' : value;
    }
    ...
    /// <param name="skipOddBytes">
    ///     If true, skip every other byte. Necessary for text which contains the color palette
    ///     association for each character.
    /// </param>
    ...
    private string DecodeText(byte[] textBytes, int step)
    {
        ...
        for (var i = 0; i < textArray.Length; i++) textArray[i] = DecodeText(textBytes[i * step]);
    ...
    private byte[] EncodeText(string text, int step, byte paletteIndex)
    {
        ...
        for (var i = 0; i < text.Length; i++)
        {
            ...
            if (step > 1) data[i * step + 1] = paletteIndex;
        }
