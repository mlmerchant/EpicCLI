...
public class Palette
{
    ...
    {
        if (data.Length != Size) throw new ArgumentException($"The palette is not {Size}-byte long.", nameof(data));

        for (var i = 0; i < _colors.Length; i++) _colors[i] = GetColor(data, i);
    }
